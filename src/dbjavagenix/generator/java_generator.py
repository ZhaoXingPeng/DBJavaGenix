...
class JavaCodeGenerator:
    ...
    def __init__(self, config: GenerationConfig):
        ...
        self.context_builder = TemplateContextBuilder(
            author=config.author, package_name=config.package_name
        )
    ...
    def generate_from_table(
        ...
        if include_dto_vo:
            ...
            for template_file in additional_templates:
                try:
                    file_path = self._generate_file(template_file, context, output_dir, "common")
                    generated_files[template_file] = file_path
    ...
    def _generate_file(
        self, template_file: str, context: Dict, output_dir: str, category: str
    ) -> str:
        """
        生成单个文件
    ...
    def _get_output_path(self, template_file: str, context: Dict, output_dir: str) -> str:
        ...
        # packageSuffix 为空时模板会产生双分隔符,在写盘边界统一归一化。
